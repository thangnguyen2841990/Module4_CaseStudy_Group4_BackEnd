...
public class MessageController {
    ...
    public ResponseEntity<List<MessageFrontend>> showAllMessages(@PathVariable Long toUserId){
        ...
        for (int i = 0; i < messages.size(); i++) {
            messages1.add(new MessageFrontend(messages.get(i).getFromUser(),messages.get(i).getToUser()
            ,postService.getDiffDays(messages.get(i).getDateCreated(),new Date()),messages.size(),messages.get(i).getStatus()));
        }
